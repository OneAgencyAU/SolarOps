...
interface GoogleStatus extends ConnectionStatus {}
interface MsStatus extends ConnectionStatus {}
...
export default function ConnectionsPage() {
  ...
  const [msStatus, setMsStatus] = useState<MsStatus>({ connected: false, email: null, lastSync: null });
  ...
  useEffect(() => {
    ...
    if (params.get('connected') === 'true') {
    ...
    } else if (params.get('ms_connected') === 'true') {
      setToast({ message: 'Microsoft 365 connected successfully', type: 'success' });
      setMsStatus(prev => ({ ...prev, connected: true }));
      window.history.replaceState({}, '', '/connections');
      ...
      setToast({ message: 'Microsoft connection failed. Please try again.', type: 'error' });
      window.history.replaceState({}, '', '/connections');
    }
  }, []);
  ...
  const handleMsConnect = () => {
    if (!tenant?.id || !user?.uid) return;
    window.location.href = `/api/auth/microsoft?tenant_id=${tenant.id}&user_id=${user.uid}`;
  };
  ...
  const handleMsDisconnect = async () => {
    ...
    try {
      ...
      setMsStatus({ connected: false, email: null, lastSync: null });
    } catch {
      setToast({ message: 'Failed to disconnect. Please try again.', type: 'error' });
    }
  };
    ...
    <div className="conn-page">
      ...
      {(googleStatus.connected || msStatus.connected) && (
        <>
          ...
          {msStatus.connected && (
            <div className="conn-active-card" style={{ marginTop: googleStatus.connected ? 12 : 0 }}>
              <div className="conn-active-left">
                <div className="conn-logo-box">
                ...
                <div className="conn-active-info">
                  <div className="conn-active-name">Microsoft 365</div>
                  <div className="conn-active-sub">Outlook · Calendar</div>
                  <div className="conn-active-account">{msStatus.email}</div>
                  <div className="conn-active-meta">
                    <span className="conn-status-pill">
                      <span className="conn-status-dot" />
                      Connected
                    </span>
                    <span className="conn-last-sync">Last synced {msStatus.lastSync ? timeAgo(msStatus.lastSync) : 'just now'}</span>
                  </div>
                </div>
              </div>
              <div className="conn-active-actions">
                <button className="conn-btn-disconnect" onClick={handleMsDisconnect}>Disconnect</button>
              </div>
            </div>
          )}
        </>
      ...
      <div className="conn-grid">
        ...
        {!msStatus.connected && !loading && (
          <div className="conn-card">
            ...
            <div className="conn-card-title">Microsoft 365</div>
