...
- `/usage` — Usage & Costs page. Header with month selector pill (← March 2026 →). Stats bar (4 cards): Total Spent This Month ($4.82), Bills Processed (247), Total API Calls (741), Budget Remaining ($45.18 with green progress bar at 9.6%). Cost by Service section: 3 cards (Google Cloud Vision/blue, Claude Haiku/purple, Claude Sonnet/orange) each showing cost, calls, per-call rate with coloured left border accent. Daily Spend line chart (recharts, 31-day data, $0–$1.00 Y-axis, #4F8EF7 line). Extraction Cost Log table: search bar + disabled Export CSV button; 10 rows (customer, retailer, Vision/Haiku/Sonnet/total cost, status pill); total row. Monthly Budget card: editable budget input, alert threshold %, estimated end-of-month ($7.20), On Track pill, progress bar, Save Budget button. All static placeholder data. Sidebar nav between Activity Log and Connections. Files: `UsagePage.tsx`, `UsagePage.css`.
