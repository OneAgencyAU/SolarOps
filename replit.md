...
- `/bill-reader` — Bill & NMI Reader page. Two-column layout: left panel (45%) with drag-and-drop upload zone, processing states (checking → extracting → complete), and recent extractions list (5 placeholder items). Right panel (55%) shows extracted bill data: Account Details with highlighted NMI (monospace, blue background), usage bar chart (peak/off-peak), tariff rates, system info pills (solar/battery/meter), collapsible raw OCR text. Copy NMI, Save extraction, Push to Simpro (disabled/coming soon) actions. Backend: 3 endpoints — POST `/api/bill-reader/check` (Google Vision + Claude Haiku classification), POST `/api/bill-reader/extract` (Google Vision + Claude Sonnet full extraction), POST `/api/bill-reader/save` (Supabase `bill_extractions` table). Packages: `@google-cloud/vision`, `multer`, `@anthropic-ai/sdk`. Migration: `006_bill_extractions.sql`. Sidebar nav between Inbox Assistant and Helpdesk.
