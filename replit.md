...
- `/activity-log` — Full activity log page. Stats bar (4 pills: actions today, pending review, errors, success rate). Filter row with search input, date range selector, and pill groups for Module (All/Voice/Inbox/Helpdesk/Connections), Type (All/AI Action/System/Error), Status (All/Success/Pending/Failed). Full-width table with 15 static rows showing Time, Module (icon+name), Action, Details, User/Trigger, Status. Row click opens slide-in drawer (420px) with full details, module badge, raw JSON log, Re-run (disabled) and Mark as Reviewed buttons. Pagination footer. All static placeholder data — no backend yet.
