...
- `POST /api/voice/assign-number` — Assigns a Telnyx number to a proper FQDN connection pointing to sip.retellai.com and imports it into Retell AI. Steps: delete old IP/FQDN connections → create FQDN connection (solarops-retell) → add sip.retellai.com:5060 FQDN target → look up number ID → PATCH connection_id → POST Retell import-phone-number → update `voice_config.telnyx_connection_id` + `telnyx_connection_type='fqdn'`.
