...
- `/helpdesk` — Kanban-based ticket management. Quick stats bar (Open/Urgent/Overdue/Resolved Today pills). Search + source/priority filter pills. 4-column Kanban board (New/In Progress/Waiting on Customer/Closed) with 13 placeholder ticket cards. Cards show ID, title, customer, source icon, priority pill, assignee, SLA status. Clicking any card opens a slide-in detail panel (480px, right side, overlay background) with: ticket header + status/priority pills, assignee dropdown, AI Summary box, and 4 tabs (Details/Transcript/Notes/Timeline). All static data — no backend yet.
- All other routes (`/connections`, `/settings`) — placeholder cards via `PlaceholderPage.tsx`
